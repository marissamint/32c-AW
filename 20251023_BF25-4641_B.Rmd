...
AUTOFLOW_destination <- "F:/Lab People/Marissa Foo/#Experiments/251105 32c UMAP"
```
...
library(scales)
library(plotly)
source(paste0(AUTOFLOW_destination, "flowPrep2.R"))
source(paste0(AUTOFLOW_destination, "utils.R"))
...
```{r}
## define path
path.input <- "/Users/flowuser/Documents/Debug/FCS/test"
...
## Load FCS files
```{r}
fs.file <- "/Users/flowuser/Documents/Reports_OUT/20251023/fcs/BF25-4641 PB Garcia_J_tubeB_B09.fcs"
fs.names <- "BF25-4641 PB Garcia_J_tubeB_B09.fcs"
analysis <- "SingleCase"
```
...
if (fs.raw == 1) {
  fs.raw <- as(lapply(fs.file, function(p1) {
    f <- read.FCS(p1, transformation = F, truncate_max_range = T)
...
comp <- f.custom_compensation(fs.raw[[1]], B_SPILL_CSV)
...
# soucing a trick file 
 library(reticulate)
use_python("C:/Users/mfoo/AppData/Local/miniconda3")
