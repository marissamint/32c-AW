---
title: "32C_B_analysis (FlowDensity)"
output:
  html_document:
    code_folding: hide
    toc: yes
...
if (!dir.exists(output_dir)) {
  ...
  message(paste("Directory exists:", output_dir))
}

# Process each sample in fs
for (i in seq_along(fs)) {
  # Create filename using parent_pop and sample identifier
  filename <- paste0(parent_pop, "_", identifier(fs[[i]]))
  
  # Create full path for output file
  output_path <- file.path(output_dir, filename)
  
  # Write FCS file to the output directory
  write.FCS(fs[[i]], output_path)
  
  # Print progress
  message(paste("Processed sample", i, "of", length(fs)))
}
